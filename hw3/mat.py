...
def matrix_vector_mul(M, v):
    ...
    assert M.D[1] == v.D
    print(M)
    #print((0,'b') in M.f.keys())
    #a=[(y,x) for y in M.D[0] for x in M.D[1]]
    #print a
    c=[]

    #a= {r:Vec(M.D[1], {c:M[r,c] for c in M.D[1]}) for r in M.D[0]}
    a= {c:Vec(M.D[0],{r:M[r,c] for r in M.D[0]}) for c in M.D[1]}
    b= [{x:y} for (x,y) in a.items()]
    c= {x:y for (x,y) in v.f.items()}
    #return zip([x.values() for x in b],
    #return a['a']
    #a= [Vec(M.D[0],[{key:a[key]*v} for key in a.keys()])]


    #print [b[y] for y in [x.keys() for x in c]]
    #[b[key]*c[key] for key in c.keys()]
    #print([y*v for y in [x for x in [Vec(M.D[1],{a:M[(b,a)] for a in [M.f] for a in M.D[1]}) for b in M.D[0]]]])
    #a= [[v[a]*M[(b,a)] for a in M.D[0]] for b in M.D[1]]
    #a= [[v[b]*M[(a,b)] for a in M.D[0]] for b in M.D[1]]
    #print(a)
    #b=[sum(a) for a in zip(a[0],a[1])]
    #c=[x for x in M.D[0]]
    #return Vec(M.D[0],{c[i]:b[i] for i in range(len(b))})
