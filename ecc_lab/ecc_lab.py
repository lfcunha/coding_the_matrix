...
R = listlist2mat([[0,0,one,0,0,0,0], [0,0,0,0,one,0,0], [0,0,0,0,0,one,0], [0,0,0,0,0,0,one]])
#print(R*(G*p))
## Task 3
...
def find_error(e):
    ...
    """
    cols={col:Vec(H.D[0], {row:H[row,col] for row in H.D[0]}) for col in H.D[1]}
    a=Vec(H.D[1],{})
    for keys in cols.keys():
        if cols[keys]==e:
            a[keys]=one
    return a
#print(find_error(Vec({0,1,2}, {0:one})))
## Task 4 part 2
